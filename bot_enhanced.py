...
def get_reply_main_menu():
    ...
    ], resize_keyboard=True)#!/usr/bin/env python3
# bot_enhanced.py
# Бот-афиша Минска
...
async def app_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    log_user_action(user.id, user.username, user.first_name, "app_command")
    await update.message.reply_text(
        "🌐 <b>Веб-афиша Минска</b>\n\n"
        "Все события удобно на одной странице — с поиском, фильтрами и календарём:\n\n"
        "👉 https://minskdvizh.lovable.app",
        parse_mode="HTML",
    )

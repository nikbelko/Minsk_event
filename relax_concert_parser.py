...
class RelaxConcertParser:
    def __init__(self, DB_PATH=os.getenv("DB_PATH", "/data/events_final.db")):
        ...
        # Список известных концертных площадок Минска
        self.known_venues = [
            'Дворец Профсоюзов',
            'Дворец Республики',
            'Минск-Арена',
    ...
    def parse_page(self, url):
        ...
        for block in event_blocks:
            try:
                ...
                if title in ['Купить билет', 'Подробнее', 'Афиша', 'Концерты']:
                    continue
        ...
        for event_key, event_data in event_dict.items():
            ...
            event = {
                ...
                'category': 'concert',  # ИСПРАВЛЕНО: 'concerts' -> 'concert'
        ...
        logging.info(f"Всего найдено концертов: {len(events)}")
        return events
    ...
    def save_events(self, events):
        ...
        logging.info(f"Добавлено новых концертов: {new_count}")
        return new_count

    def run(self):
        logging.info("=" * 60)
        logging.info("🎵 ПАРСЕР КОНЦЕРТОВ")
        logging.info("=" * 60)
        ...
        if events:
            ...
            print(f"   🧹 Очищено {len(events)} старых записей концертов")
            print(f"   📊 Результаты:")
            print(f"      ✅ Добавлено новых концертов: {saved}")
        else:
            logging.warning("Концерты не найдены")
            print(f"   ⚠️ Событий концертов не найдено")
